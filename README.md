...
- 応募部門：次の3部門から1つ選択
  ...
  - スイッチカードOFF部門
  - 初心者部門（スイッチカードOFF必須）
- 評価値・最終所持スキルカード画像：必須。評価値と最終所持スキルカードを同時に確認できる1枚を添付

初心者部門の参加条件は、イベント参加時点で「PLv60未満」または「ログイン日数合計90日以下」です。初心者部門を選択した場合に限り、PIDとPレベルの両方が分かる画像を必須とします。
...
- 育成キャラクターまたは確認状態による絞り込み
- 最終更新日時、回答日時、確認済み評価値、ユーザー名による並び替え
- 評価値・最終所持スキルカード画像、初心者用PID・Pレベル画像の確認
...
パスは `${auth.uid()}/score/${UUID}.ext`、初心者部門のみ `${auth.uid()}/beginner-proof/${UUID}.ext` です。`deck` パスは旧形式の回答との移行互換性のためだけに残されています。RLSは所有者の作成・閲覧・更新・削除、管理者の閲覧だけを許可します。管理画面は5分間有効なSigned URLを発行します。
...
- [ ] 初心者部門でだけPID・Pレベル画像が必須になり、管理者が確認できる
