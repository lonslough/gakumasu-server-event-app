import { corsHeaders, generatePassword, json, normalizeAndValidateUserId, requireAdmin } from '../_shared/admin.ts'
...
Deno.serve(async (request) => {
  ...
  const { data, error } = await admin.auth.admin.createUser({
    email: `${userId}@app.invalid`, password, email_confirm: true,
  })
  if (error || !data.user) return json({ error: 'create_failed' }, 500)
  const { error: profileError } = await admin.from('profiles').insert({ id: data.user.id, user_id: userId, role: 'user' })
  if (profileError) {
